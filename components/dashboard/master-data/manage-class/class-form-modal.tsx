...
export function ClassModal({ isOpen, onClose, onSubmit, initialData, mode }: ClassModalProps) {
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      toast({
        ...
        description: `Class ${formData.classCode} has been ${mode === "add" ? "added" : "updated"}.`,
      })
      onClose()
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{mode === "add" ? "Add New Class" : "Edit Class"}</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit}>{mode === "add" ? "Add Class" : "Save Changes"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
