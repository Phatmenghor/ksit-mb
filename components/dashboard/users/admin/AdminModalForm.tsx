...
import { AdminFormData, AdminFormSchema } from "@/model/user/staff/schema";
...
import { AdminEditFormSchema } from "@/model/user/staff/zod.staff.model";
...
export default function AdminModalForm({
  ...
  const [selectedAdmin, setSelectedAdmin] = useState<StaffModel | null>(
    initialData?.selectedAdmin || null
  );
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData && mode === "edit") {
        ...
        if (initialData.selectedAdmin) {
          setSelectedAdmin(initialData.selectedAdmin);
        }
      } else {
        // Reset for add mode
        form.reset({
  ...
  const handleSubmit = async (data: AdminFormData) => {
    try {
      ...
      if (mode === "add") {
        submitData.password = cleanRequired((data as AdminFormData).password);
        submitData.confirmPassword = cleanRequired(
          (data as AdminFormData).confirmPassword
        );
      }
