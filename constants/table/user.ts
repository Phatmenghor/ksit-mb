export const StaffTableHeader = [
  { label: "#", className: "w-[10%]" },
  { label: "Username", className: "w-[20%]" },
  { label: "Fullname (KH)", className: "w-[30%]" },
  { label: "Fullname (EN)", className: "w-[30%]" },
  { label: "Gender", className: "w-[20%]" },
  { label: "Actions", className: "w-[10%]" },
];

export const AdminTableHeader = [
  { label: "#", className: "w-[10%]" },
  { label: "Username", className: "w-[20%]" },
  { label: "Email", className: "w-[30%]" },
  { label: "Fullname", className: "w-[30%]" },
  { label: "Actions", className: "w-[10%]" },
];

export const TeacherTableHeader = [
  { label: "#", className: "w-[5%]" },
  { label: "Username", className: "w-[20%]" },
  { label: "Fullname (KH)", className: "w-[30%]" },
  { label: "Fullname (EN)", className: "w-[30%]" },
  { label: "Teacher ID", className: "w-[20%]" },
  { label: "Gender", className: "w-[20%]" },
  { label: "Date Of Birth", className: "w-[10%]" },
  { label: "Phone Number", className: "w-[10%]" },
  { label: "Department", className: "w-[10%]" },
  { label: "Actions", className: "w-[4%]" },
];

export const StudentTableHeader = [
  { label: "#", className: "w-[10%]" },
  { label: "Username", className: "w-[20%]" },
  { label: "Fullname (KH)", className: "w-[30%]" },
  { label: "Fullname (EN)", className: "w-[30%]" },
  { label: "Gender", className: "w-[20%]" },
  { label: "Date Of Birth", className: "w-[10%]" },
  { label: "Class code", className: "w-[10%]" },
  { label: "Actions", className: "w-[10%]" },
];

export const StudentSurveyHeader = [
  { label: "#", className: "" },
  { label: "Username", className: "" },
  { label: "Fullname (KH)", className: "" },
  { label: "Fullname (EN)", className: "" },
  { label: "Gender", className: "]" },
  { label: "Date Of Birth", className: "" },
  { label: "Class code", className: "" },
  { label: "Status", className: "" },
];

export const StudentListTableHeader = [
  { label: "#", className: "w-[10%]" },
  { label: "Username", className: "w-[20%]" },
  { label: "Fullname (KH)", className: "w-[30%]" },
  { label: "Fullname (EN)", className: "w-[30%]" },
  { label: "Student Id", className: "w-[10%]" },
  { label: "Gender", className: "w-[20%]" },
  { label: "Date Of Birth", className: "w-[10%]" },
  { label: "Class code", className: "w-[10%]" },
];

export const StudentAttendanceTableHeader = [
  { label: "#", className: "w-[10%]" },
  { label: "Student ID", className: "w-[20%]" },
  { label: "Student Name", className: "w-[30%]" },
  { label: "Attendance", className: "w-[30%]" },
  { label: "Type", className: "w-[20%]" },
  { label: "Check-in Time", className: "w-[10%]" },
  { label: "Comments", className: "w-[10%]" },
];
